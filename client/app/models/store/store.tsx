...
export class Store<T extends Storable<J>, J extends StorableJSON> {
  ...
  constructor(private Klass: { new(incoming: Partial<J>): T, schema: string }, indices: string[] = []) {
  ...
  upsertList(incoming: Partial<J>[]) {
    const list = incoming.map(f => this.upsert(f));
    return list;
  }
  ...
  removeWhere(filter: (f: T) => boolean) {
    this.db.values().forEach(v => {
      if (filter(v)) this.remove(v)
    });
  }
